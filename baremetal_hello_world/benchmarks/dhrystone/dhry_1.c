...
// uint32_t riscv_get_instret(void) {
//     uint32_t ret = 0;
//     asm volatile("csrr %0, instret":"=r"(ret));
//     return ret;
// }

// uint64_t riscv_get_current_cycle(void) {
  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  /* Start timer */
  /***************/
  //uint64_t start_cycle = riscv_get_current_cycle();
  //uint32_t start_instret = riscv_get_instret();
  ...
  /* Stop timer */
  /**************/
  //uint32_t end_instret = riscv_get_instret();
  //uint64_t end_cycle = riscv_get_current_cycle();
  ...
  printf("start_instret :%#x end_instret:%#x duration:%#d\n", start_instret, end_instret, duration_instret);
  printf("start_cycle upper:%#x lower:%#x\n ", (uint32_t)(start_cycle>>32), (uint32_t)start_cycle);
  printf("end_cycle upper:%#x lower:%#x\n", (uint32_t)(end_cycle>>32), (uint32_t)end_cycle);
  printf("duration upper:%#x lower:%#x\n", (uint32_t)(duration_cycle>>32), (uint32_t)duration_cycle);
  printf("Cycle Duration: %ld\n", duration_cycle);

  printf("Assuming CPU frequency is (Hz): %d\n", CPU_FREQ_HZ);
  
  printf("IPC: %f\n", (float)duration_instret / duration_cycle);
  printf("DMIPS/MHz: %f\n", (float)Number_Of_Runs / duration_cycle * 1000000 / 1757);
  Microseconds = duration_cycle  / (CPU_FREQ_HZ / 1000000 * Number_Of_Runs);
  Dhrystones_Per_Second = Number_Of_Runs * (CPU_FREQ_HZ / duration_cycle);
